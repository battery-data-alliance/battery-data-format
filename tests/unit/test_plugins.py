"""Unit tests for the Plugin model (bdf.plugins).

Covers JSON round-trip, match_ext/match_magic, score, EXT_TO_READER
single-valuedness, and Plugin construction guards.
"""

from __future__ import annotations

import pytest

import bdf.plugins as P
from bdf.normalizers import NORMALIZERS, Normalizer, Syn
from bdf.plugins import (
    PLUGINS,
    Plugin,
    build_ext_to_reader,
)
from bdf.readers import DelimTxtReader, ExcelReader


@pytest.mark.parametrize("ds", list(PLUGINS.values()), ids=list(PLUGINS))
def test_plugin_json_round_trip(ds: Plugin) -> None:
    """Every built-in Plugin survives model_dump_json → model_validate_json."""
    assert Plugin.model_validate_json(ds.model_dump_json()) == ds


def test_match_ext() -> None:
    """match_ext is true only for the source's distinctive (case-insensitive) extensions."""
    assert P.BIOLOGIC_MPT.match_ext(".mpt")
    assert P.BIOLOGIC_MPT.match_ext(".MPT")
    assert not P.BIOLOGIC_MPT.match_ext(".csv")
    assert not P.ARBIN_CSV.match_ext(".csv")  # arbin declares no distinctive exts


def test_match_magic_text() -> None:
    """match_magic matches case-insensitive string tokens against decoded head text."""
    assert P.MACCOR_CSV.match_magic(b"...Date of Test:,2021...")
    assert not P.MACCOR_CSV.match_magic(b"nothing relevant here")
    assert not P.ARBIN_CSV.match_magic(b"anything")  # no magic declared


def test_match_magic_bytes_token() -> None:
    """match_magic supports raw byte tokens (binary-safe), not just str tokens."""
    ds = Plugin(id="bin", magic=(b"\x89PNG",), normalizer=Normalizer(), reader=DelimTxtReader())
    assert ds.match_magic(b"\x89PNG\r\n\x1a\n rest")
    assert not ds.match_magic(b"plain text")


def test_score_delegates_to_normalizer() -> None:
    """Plugin.score counts headers resolved by the referenced normalizer."""
    ds = Plugin(id="x", normalizer=Normalizer(voltage_volt=[Syn("v")]), reader=DelimTxtReader())
    assert ds.score(["v"]) == 1
    assert ds.score(["nope"]) == 0


def test_neware_csv_and_xlsx_share_normalizer() -> None:
    """NEWARE_CSV and NEWARE_XLSX reference one shared normalizer (different engines)."""
    assert P.NEWARE_CSV.normalizer is P.NEWARE_XLSX.normalizer is NORMALIZERS["neware"]
    assert P.NEWARE_CSV.reader.name == "txt"
    assert P.NEWARE_XLSX.reader.name == "excel"


def test_binary_engine_magic_warns() -> None:
    """Setting magic/metadata on a binary-engine source warns (text-only relevance)."""
    with pytest.warns(UserWarning, match="binary reader"):
        Plugin(id="b", magic=("foo",), normalizer=Normalizer(), reader=ExcelReader())


def test_binary_reader_default_metadata_no_warn() -> None:
    """Binary-reader source with no magic and default metadata fields does not warn."""
    import warnings

    with warnings.catch_warnings():
        warnings.simplefilter("error", UserWarning)
        Plugin(id="b", normalizer=Normalizer(), reader=ExcelReader())


def test_conflicting_extension_raises() -> None:
    """A fixture where two readers claim the same extension raises ValueError."""
    csv_src = Plugin(id="c", exts=(".zct",), normalizer=Normalizer(), reader=DelimTxtReader())
    xls_src = Plugin(id="x", exts=(".zct",), normalizer=Normalizer(), reader=ExcelReader())
    with pytest.raises(ValueError, match="claimed by readers"):
        build_ext_to_reader({"c": csv_src, "x": xls_src})
