...
from bdf.derive import DERIVABLE_COLUMNS, DERIVABLE_STEP_COLUMNS, derive
...
def test_step_count_is_monotonic_execution_counter():
    """Step Count: 1-based monotonic counter, +1 per new step, never resets/repeats —
    even when the vendor Step ID recurs (here 1 -> 2 -> 1 = three executions)."""
    df = _df(t=[0, 1, 2, 3, 4, 5], v=[3.7]*6, i=[1.0]*6, step_id=[1, 1, 2, 2, 1, 1])
    out = derive(df)
    np.testing.assert_array_equal(out["Step Count / 1"], [1, 1, 2, 2, 3, 3])


def test_step_index_resets_within_each_step():
    """Step Index: 1-based positional counter for points *within* a step; resets each step."""
    df = _df(t=[0, 1, 2, 3, 4, 5], v=[3.7]*6, i=[1.0]*6, step_id=[1, 1, 2, 2, 1, 1])
    out = derive(df)
    np.testing.assert_array_equal(out["Step Index / 1"], [1, 2, 1, 2, 1, 2])


def test_step_count_and_step_index_are_distinct():
    """Guard against the historical Step Count / Step Index / Step ID confusion."""
    df = _df(t=[0, 1, 2, 3], v=[3.7]*4, i=[1.0]*4, step_id=[5, 5, 9, 9])
    out = derive(df)
    # Step ID values are 5,5,9,9 (vendor); Count is global+monotonic; Index resets.
    np.testing.assert_array_equal(out["Step Count / 1"], [1, 1, 2, 2])
    np.testing.assert_array_equal(out["Step Index / 1"], [1, 2, 1, 2])
