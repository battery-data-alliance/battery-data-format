...
from bdf.fetch import URLFileExtensionError, _safe_cache_name
...
class TestSafeCacheName:
    ...
    def test_no_extension_anywhere_raises(self) -> None:
        with pytest.raises(URLFileExtensionError, match="Cannot determine file type"):
            _safe_cache_name("https://example.com/api/data", None)

    def test_no_extension_error_message_mentions_filename_param(self) -> None:
        with pytest.raises(URLFileExtensionError, match="filename="):
