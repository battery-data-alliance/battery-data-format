...
def _clean_fields(fields: List[str]) -> List[str]:
    # Strip whitespace and remove accidental leading/trailing empty fields
...
def _read_landt_csv_robust(path: Path, header_idx: int, delim: str, fields: List[str]) -> pd.DataFrame:
    ...
    Robust line-by-line reader:
      ...
      - split each row; if too long, fold extras into the last column (e.g., step_name)
      - trim a trailing delimiter introduced by a trailing comma
    """
    ncols = len(fields)
    rows: List[List[str]] = []
    ...
    with fh:
        for i, raw in enumerate(fh):
            ...
            # Try csv.reader first (handles quoted fields)
            try:
            ...
            if len(parts) > ncols:
                head = parts[: ncols - 1]
                tail = delim.join(parts[ncols - 1 :])  # join the remainder
                # Remove a dangling trailing delimiter (e.g., "rest,")
                tail = tail.rstrip(delim).rstrip()     # "rest," -> "rest"
                parts = head + [tail]

            # If row has exactly one extra empty last field (trailing comma), fix it
            elif len(parts) == ncols + 1 and parts[-1] == "":
                parts = parts[:-1]  # drop the empty last field
...
class LandtCSV(CyclerPlugin):
    ...
    def detect(self, path: Path) -> SniffResult:
        ...
        try:
            ...
            return SniffResult(self.id, 0.6, "CSV extension; Landt fields not confirmed")
    ...
    def parse(self, path: Path) -> pd.DataFrame:
        ...
        # Always use the robust reader to avoid silent misalignment in pandas fast path
