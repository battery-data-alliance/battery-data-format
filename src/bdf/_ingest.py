# src/bdf/_ingest.py
"""Batch ingestion: convert raw vendor files to BDF and validate existing BDF artifacts."""
...
from ._source import _is_url, _looks_like_bdf_artifact
from .validate import BDFValidationError

# ---------------------------------------------------------------------------
# Internal helpers (previously nested inside ingest())
# ---------------------------------------------------------------------------
...
def ingest(
    ...
    *,
    ...
    """
    # Avoid circular import: read/validate are in the top-level __init__
    import bdf as _bdf
    ...
    def _doi_request_json(url: str) -> Optional[dict]:
        try:
            ...
            return None
        headers = {
            "User-Agent": f"bdf/{_bdf.__version__}",
        ...
        try:
            ...
            if _looks_like_bdf_artifact(f):
                ...
                if validate_existing:
                    rep = _bdf.validate(output_used, report=False, raise_on_error=False)
