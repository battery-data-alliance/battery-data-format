...
### Added
- Offline ontology suggestions for unlinked additional columns in CLI conversion and file validation. `bdf terms` reviews candidates, saves explicit selections to metadata, and exports reusable mapping profiles. The bundled BattINFO index is pinned; links do not trigger live fetching or unit conversion. CLI conversion now carries measurement metadata into its output sidecar. See [Finding ontology terms](docs/ontology-terms.md).
...
- Reference-data manifest hardening: `datasets.json` sha256s now hash the stored artifact bytes on every platform (`.gitattributes` pins the artifacts `-text`; ten entries previously hashed a Windows CRLF rendering and failed to verify on Linux), a CI test now enforces manifest/file agreement, the manifest gains `manifest_version` and records the canonical artifact record's DOI (10.5281/zenodo.22230066, publishes with the tag), and the Stanford unconverted-file reason is corrected.
- A reserved sidecar that fails validation now raises `BDFMetadataError` (previously a raw pydantic `ValidationError`); where the document claims a newer `bdf_version` than the installed package, the message says which version wrote it and to upgrade (GH #106).
